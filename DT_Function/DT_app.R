...
### saveRDS(DT3, "./note.rds")
  ...
  output$MainBody_trich<-renderUI({
    fluidPage(
      column(6,
             h4("F2 Ingredient Database",style="font-weight:520;"),
             helpText("Add or Remove Available Ingredients")),
      column(6,
  ...
  output$Main_table_trich<-renderDataTable({
    DT=vals_trich$Data
    DT$Ingredient_costs <- dollar(DT$Ingredient_costs)
    datatable(DT,selection = 'single',
              escape=F,colnames = c("Ingredient","Amount per Gal (in Units)","Units","Cost per Unit")) 
  })
  ...
  output$Trich_csv<- downloadHandler(
    filename = function() {
      paste("Booch_F2_Ingredients", Sys.Date(), ".csv", sep="")
    },
